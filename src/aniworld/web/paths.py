...
def series_download_path():
    """Optional override for series downloads. Empty when unset."""
    raw = os.environ.get("ANIWORLD_SERIES_DOWNLOAD_PATH", "").strip()
    return expand(raw) if raw else None


def movie_download_path():
    """Optional override for movie downloads. Empty when unset."""
    raw = os.environ.get("ANIWORLD_MOVIE_DOWNLOAD_PATH", "").strip()
    return expand(raw) if raw else None
