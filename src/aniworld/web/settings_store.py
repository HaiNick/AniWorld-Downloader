...
# Download paths are written through to .env as well: a container that forgot
# where to put downloads on every restart would be useless.
...
def _persist_paths(updates):
    subset = {k: v for k, v in updates.items() if k in PATH_ENV_KEYS}
    if not subset:
        return
    try:
        from ..env import persist_env_values
        ...
        logger.warning("Could not persist path settings to .env: %s", exc)
...
# ---------------------------------------------------------------------------
def read_settings():
    series_path = paths.series_download_path()
    movie_path = paths.movie_download_path()
    return {
        "download_path": str(paths.default_download_path()),
        "series_download_path": str(series_path) if series_path else "",
        "movie_download_path": str(movie_path) if movie_path else "",
...
def update_settings(data):
    ...
    if "series_download_path" in data:
        updates["ANIWORLD_SERIES_DOWNLOAD_PATH"] = str(
            data["series_download_path"]
        ).strip()
    if "movie_download_path" in data:
        updates["ANIWORLD_MOVIE_DOWNLOAD_PATH"] = str(
            data["movie_download_path"]
        ).strip()
